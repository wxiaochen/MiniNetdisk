...
//        MultipartRequest mr = new MultipartRequest(request, "c://upload",1024 * 1024 * 10);
//    }
    public String addImage(HttpServletRequest request, String path1, String path2) {
        //设置目录
        ...
        *这里我就先不做多的介绍。
        */
            File file1 = this.getFile(imgFile1,path1,path2,fileTypes);
    ...
    private File getFile(MultipartFile imgFile,String typeName,String brandName,List fileTypes) {
        ...
        if(fileTypes.contains(ext)) {                      //如果扩展名属于允许上传的类型，则创建文件
            file = this.creatFolder(typeName, brandName, "img."+ext);
            try {
